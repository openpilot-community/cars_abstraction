  ...
  module Util
    ...
    def self.object_classes
      @object_classes ||= {
        ...
        # business objects
        Vehicle::OBJECT_NAME => Vehicle
      }
    end

    # Converts a hash of fields or an array of hashes into a +LegacyObject+ or
    # array of +LegacyObject+s. These new objects will be created as a concrete
    # type as dictated by their `object` field (e.g. an `object` value of
    # `charge` would create an instance of +Story+), but if `object` is not
    # present or of an unknown type, the newly created instance will fall back
    # to being a +LegacyObject+.
    #
    # ==== Attributes
    #
    # * +data+ - Hash of fields and values to be converted into a LegacyObject.
    # * +opts+ - Options for +LegacyObject+ like an API key that will be reused
    #   on subsequent API calls.
    def self.convert_to_legacy_object(data, opts = {})
      case data
      when Array
        data.map { |i| convert_to_legacy_object(i, opts) }
      when Hash
        # Try converting to a known object class.  If none available, fall back to generic LegacyObject
        object_classes.fetch(data[:object], LegacyObject).construct_from(data, opts)
      else
        data
      end
    end
    ...
    def self.log_error(message, data = {})
      if !Cars.logger.nil? ||
         !Cars.log_level.nil? && Cars.log_level <= Cars::LEVEL_ERROR
        log_internal(message, data, color: :cyan,
                                    level: Cars::LEVEL_ERROR, logger: Cars.logger, out: $stderr)
      end
    end

    def self.log_info(message, data = {})
      if !Cars.logger.nil? ||
         !Cars.log_level.nil? && Cars.log_level <= Cars::LEVEL_INFO
        log_internal(message, data, color: :cyan,
                                    level: Cars::LEVEL_INFO, logger: Cars.logger, out: $stdout)
      end
    end

    def self.log_debug(message, data = {})
      if !Cars.logger.nil? ||
         !Cars.log_level.nil? && Cars.log_level <= Cars::LEVEL_DEBUG
        log_internal(message, data, color: :blue,
                                    level: Cars::LEVEL_DEBUG, logger: Cars.logger, out: $stdout)
      end
    end
